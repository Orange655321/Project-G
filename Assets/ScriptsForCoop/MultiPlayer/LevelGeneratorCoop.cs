...
using UnityEngine;
using Photon.Pun;
public class LevelGeneratorCoop : MonoBehaviourPunCallbacks
{
    ...
    {  // if (!PhotonNetwork.IsMasterClient)  return;
        Random.InitState(LobbyManager.keyOfMap);
        Setup();
        CreateFloors();
        CreateWalls();
        RemoveSingleWalls();
    ...
    void Spawn(float x, float y, int num)
    {
        ...
        {
            case 0:
                Instantiate(floorObj, spawnPos, Quaternion.identity);
                break;
            case 1:
                Instantiate(wallObj, spawnPos, Quaternion.identity);
                break;
        }
